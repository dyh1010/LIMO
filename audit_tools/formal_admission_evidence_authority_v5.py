"""Out-of-band trust-root wrapper for the v6 BLOCKED_OFFLINE authority.
...
CORE_SOURCE_RELATIVE_PATH = (
    "audit_tools/formal_admission_evidence_authority_v5_core.py"
)

# This may be frozen after the core bytes are stable.  It is separate from the
# index trust anchor and does not select an evidence generation by itself.
...
_WINDOWS_PRODUCTION_INDEX_TRUST_ANCHOR: Mapping[str, Any] = {
    "path": (
...
def _load_exact_core(
    ...
    if not isinstance(anchor, dict):
        return None, {}, ["formal_authority_v5_core_source_anchor_not_configured"]
    if set(anchor) != {"path", "size_bytes", "sha256"}:
        return None, {}, ["formal_authority_v5_core_source_anchor_invalid"]
    if anchor.get("path") != CORE_SOURCE_RELATIVE_PATH:
        return None, {}, ["formal_authority_v5_core_source_anchor_path_mismatch"]
    ...
    try:
        ...
        return None, {}, ["formal_authority_v5_core_source_unreadable"]
    identity = {
    ...
    failures = [
        "formal_authority_v5_core_source_" + key + "_mismatch"
        for key in ("size_bytes", "sha256")
        if identity[key] != anchor.get(key)
    ]
    if failures:
        return None, identity, failures
    private_name = "_ros1_formal_authority_v5_core_exact_" + identity["sha256"]
    ...
    try:
        ...
        return None, identity, ["formal_authority_v5_core_source_execution_failed"]
    finally:
        if previous is None:
    ...
    if module.__file__ != str(path) or any(
        ...
        return None, identity, ["formal_authority_v5_core_source_api_invalid"]
