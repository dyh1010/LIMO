"""Versioned validator for the v7 field-producer BLOCKED_OFFLINE authority.
...
GENERATION_ID = "ros1_atomic_cli_field_producer_contract_blocked_offline_20260816_v7"
GENERATION_SCOPE = (
    "blocked_offline_atomic_cli_and_field_producer_contract_"
...
CANONICAL_ID = (
    ...
    "v8-atomic-cli-field-producer-blocked-offline"
)
CANONICAL_ARTIFACT_ID = (
    "ros1_noetic_canonical_source_admission_20260816_"
    "v8_atomic_cli_field_producer_blocked_offline"
)
REPORT_ID = (
    "ros1-atomic-cli-field-producer-contract-blocked-offline-"
    "report-20260816-v7"
)
SELECTION_AUTHORITY = (
...
PREDECESSOR_INDEX_IDENTITY: Mapping[str, Any] = {
    ...
    "size_bytes": 34222,
    "sha256": "af92de59762b5c08fbf432c7f31ea6141d2a72e09d8a856df37034e131ab68f6",
...
PREDECESSOR_REPORT_IDENTITY: Mapping[str, Any] = {
    ...
    "size_bytes": 854414,
    "sha256": "82227de8917d19cfc5208edd27937d6dbbc7f4e9a9d65cfb1b59eea8d522d905",
}
PREDECESSOR_CANONICAL_IDENTITY: Mapping[str, Any] = {
    ...
    "size_bytes": 49152,
    "sha256": "bebd72c1723ddbe79b292a8f2f71c2d80fa95f6f0f4d27a303fe919f4a152e3c",
}
...
GENERATION_WRAPPER_SOURCE_PATH = (
    "audit_tools/formal_admission_evidence_authority_v6.py"
)
...
REQUIRED_SOURCE_ROLE_DEFINITIONS: Tuple[Tuple[str, str, str], ...] = (
    ...
    *tuple(
    ...
    ("successor_evidence_generator", "workspace", "audit_tools/generate_ros1_atomic_cli_field_producer_contract_blocked_offline_evidence_v1.py"),
    ("successor_authority_core", "workspace", "audit_tools/formal_admission_evidence_authority_v6_core.py"),
    ("successor_authority_test", "workspace", "audit_tools/test_formal_admission_evidence_authority_v6.py"),
    ("predecessor_authority_resolver", "workspace", "audit_tools/formal_admission_evidence_authority_v5.py"),
    ("predecessor_authority_core", "workspace", "audit_tools/formal_admission_evidence_authority_v5_core.py"),
...
FROZEN_REQUIRED_SOURCE_IDENTITIES: Mapping[Tuple[str, str], Mapping[str, Any]] = {
    ...
    ("workspace", "audit_tools/formal_admission_evidence_authority_v5.py"): {
        "root_role": "workspace",
        "path": "audit_tools/formal_admission_evidence_authority_v5.py",
        "size_bytes": 8805,
        "sha256": "c35ce416a9d1b8aa2c328a533477a3cb5cfaa834caded8c1203e86a350a45159",
    },
    ("workspace", "audit_tools/formal_admission_evidence_authority_v5_core.py"): {
        "root_role": "workspace",
        "path": "audit_tools/formal_admission_evidence_authority_v5_core.py",
        "size_bytes": 113149,
        "sha256": "04e432bc7bbd25c3b98e26d12e8f8ae9511ba1108a556edabb13fa88f4e697e9",
    },
}
...
SUITE_DEFINITIONS: Tuple[Mapping[str, str], ...] = (
    ...
    {"suite_id": "successor_authority_validator", "root_role": "workspace", "target": "audit_tools/test_formal_admission_evidence_authority_v6.py", "runner": "unittest"},
)
...
PRODUCTION_INTERPRETER_ATTESTATION = (
    ...
    "def _interpreter_metadata(_value):\n"
    ...
    "getattr(_value,'st_nlink',1))\n"
    "try:\n"
    "    _interpreter_before=os.lstat(_interpreter_manifest['path'])\n"
    "    if (stat.S_ISLNK(_interpreter_before.st_mode) or "
    "not stat.S_ISREG(_interpreter_before.st_mode)): raise SystemExit(107)\n"
    ...
    "_interpreter_metadata(_interpreter_opened_before) != "
    "_interpreter_metadata(_interpreter_before)): raise SystemExit(107)\n"
    "        _interpreter_chunks=[]\n"
    "        while True:\n"
    ...
    "if (_interpreter_metadata(_interpreter_opened_after) != "
    "_interpreter_metadata(_interpreter_opened_before) or "
    "_interpreter_metadata(_interpreter_after) != "
    "_interpreter_metadata(_interpreter_before)): raise SystemExit(107)\n"
    "_interpreter_raw=b''.join(_interpreter_chunks)\n"
    "if (len(_interpreter_raw) != _interpreter_manifest['size_bytes'] or "
...
PRODUCTION_SCRIPT_BOOTSTRAP = (
    ...
    "def _metadata(_value):\n"
    "    return (getattr(_value,'st_dev',None),getattr(_value,'st_ino',None),"
    "_value.st_mode,_value.st_size,getattr(_value,'st_mtime_ns',None),"
    "getattr(_value,'st_nlink',1))\n"
    "try:\n"
    "    _before=os.lstat(_target)\n"
    "    if (stat.S_ISLNK(_before.st_mode) or "
    ...
    "_metadata(_opened_before) != _metadata(_before)): raise SystemExit(103)\n"
    "        _chunks=[]\n"
    "        while True:\n"
    ...
    "if (_metadata(_opened_after) != _metadata(_opened_before) or "
    "_metadata(_after) != _metadata(_before)): raise SystemExit(103)\n"
    "_raw=b''.join(_chunks)\n"
    "if (len(_raw) != _entry['size_bytes'] or "
...
PRODUCTION_PACKAGE_BOOTSTRAP = (
    ...
    "            if (not stat.S_ISREG(_opened_before.st_mode) or "
    "getattr(_opened_before,'st_nlink',1) != 1 or "
    "_metadata(_opened_before) != _metadata(_before)): "
    "raise SystemExit(103)\n"
    "            _chunks=[]\n"
    "            while True:\n"
    "                _chunk=os.read(_descriptor,1024*1024)\n"
...
def _policy_failures(policy: Any) -> List[str]:
    if not isinstance(policy, AuthorityPolicy):
        return ["formal_authority_v6_policy_invalid"]
    failures: List[str] = []
    for label, path in (
        ...
        if not _safe_relative(path):
            failures.append("formal_authority_v6_policy_path_invalid:" + label)
    ...
    if not roles or any(
        ...
        failures.append("formal_authority_v6_policy_source_roles_invalid")
    elif (
        len({item[0] for item in roles}) != len(roles)
        or len({(item[1], item[2]) for item in roles}) != len(roles)
    ):
        failures.append("formal_authority_v6_policy_source_roles_duplicate")
    if all(isinstance(item, tuple) and len(item) == 3 for item in roles) and any(
        ...
        failures.append("formal_authority_v6_policy_nonformal_diagnostic_forbidden")
    if policy.host_perception_package_root is None:
        if (
            ...
            failures.append(
                "formal_authority_v6_policy_host_package_inventory_invalid"
            )
    elif (
        ...
        failures.append(
            "formal_authority_v6_policy_host_package_source_roles_incomplete"
        )
    elif (
        ...
        failures.append(
            "formal_authority_v6_policy_host_package_cache_inventory_invalid"
        )
    elif {
        (
            "workspace",
        ...
        failures.append(
            "formal_authority_v6_policy_host_package_cache_roles_incomplete"
        )
    if (
        ...
        failures.append(
            "formal_authority_v6_policy_allowed_empty_sources_invalid"
        )
    suite_ids = [item.get("suite_id") for item in policy.suite_definitions]
    if not suite_ids or len(suite_ids) != len(set(suite_ids)):
        failures.append("formal_authority_v6_policy_suite_inventory_invalid")
    ...
    if not execution_ids or len(execution_ids) != len(set(execution_ids)):
        failures.append("formal_authority_v6_policy_execution_inventory_invalid")
    if (
        not isinstance(
        ...
        failures.append(
            "formal_authority_v6_policy_predecessor_lineage_invalid"
        )
    return failures
...
def _workspace_loader_marker_failures(
    ...
    prefix = "formal_authority_v6_workspace_loader:" + record_id
    failures: List[str] = []
    ...
    if marker.get("workspace_pyc_inode_policy") != (
...
def _external_wrapper_observation_failures(
    ...
    prefix = "formal_authority_v6_external_wrapper:" + str(record_id)
...
def _validate_physical_records(
    ...
    if not isinstance(value, list):
        return ["formal_authority_v6_physical_records_invalid"], {}
    ...
    try:
        ...
        failures.append(
            "formal_authority_v6_workspace_loader_allowed_sources_invalid"
        )
    ...
    required_keys = {
        ...
        "environment_sha256",
        "stdout", "stderr", "external_wrapper_observation",
    }
    for item in value:
        if not isinstance(item, dict) or set(item) != required_keys:
            failures.append("formal_authority_v6_physical_record_schema_invalid")
            continue
        record_id = item.get("record_id")
        if not isinstance(record_id, str) or not record_id:
            failures.append("formal_authority_v6_physical_record_id_invalid")
            continue
        if record_id in by_id:
            failures.append("formal_authority_v6_physical_record_duplicate:" + str(record_id))
            continue
        by_id[record_id] = item
        definition = definitions.get(record_id)
        if definition is None:
            failures.append("formal_authority_v6_physical_record_unknown:" + str(record_id))
            continue
        suite = suites.get(definition["suite_id"])
        if suite is None:
            failures.append("formal_authority_v6_physical_suite_missing:" + str(record_id))
            continue
        ...
        for key, expected in expected_scalars.items():
            if not _same(item.get(key), expected):
                failures.append("formal_authority_v6_physical_record_mismatch:{}:{}".format(record_id, key))
        ...
        try:
            ...
            expected_argv = None
            failures.append("formal_authority_v6_physical_argv_recompute_failed:" + record_id)
        if expected_argv is None or not _same(item.get("argv"), expected_argv):
            failures.append("formal_authority_v6_physical_argv_mismatch:" + record_id)
        environment = item.get("environment")
        if definition["platform"] == "POSIX_WSL":
            if not _same(environment, dict(CHILD_ENVIRONMENT)):
                failures.append("formal_authority_v6_physical_environment_mismatch:" + record_id)
        elif (
            ...
            failures.append("formal_authority_v6_windows_environment_invalid:" + record_id)
        marker = item.get("marker_payload")
        if not isinstance(marker, dict) or item.get("marker_payload_sha256") != _canonical_sha256(marker):
            failures.append("formal_authority_v6_marker_payload_invalid:" + record_id)
        else:
            for key, expected in (
                ("path", suite["target"]),
                ...
                if not _same(marker.get(key), expected):
                    failures.append("formal_authority_v6_marker_mismatch:{}:{}".format(record_id, key))
            if (
                suite["runner"] == "unittest"
                and not _same(marker.get("executable"), item.get("interpreter_identity"))
            ):
                failures.append(
                    "formal_authority_v6_marker_mismatch:{}:executable".format(
                        record_id
                    )
                )
        ...
        for key, material in (
            ...
            if not _valid_sha256(item.get(key)):
                failures.append("formal_authority_v6_physical_hash_invalid:{}:{}".format(record_id, key))
            elif item[key] != _canonical_sha256(material):
                failures.append("formal_authority_v6_physical_hash_mismatch:{}:{}".format(record_id, key))
        if not _stream_identity_valid(item.get("stdout")) or not _stream_identity_valid(item.get("stderr")):
            failures.append("formal_authority_v6_physical_stream_identity_invalid:" + record_id)
    failures.extend(_external_wrapper_observation_set_failures(
        by_id, definitions,
    ))
    if set(by_id) != set(definitions):
        failures.append("formal_authority_v6_physical_record_set_invalid")
    if value != sorted(
        value,
        key=lambda item: (
            item.get("record_id")
        ...
        failures.append("formal_authority_v6_physical_record_order_invalid")
    ...
    for record_id, item in by_id.items():
        ...
        if role in role_identities and role_identities[role] != identity:
            failures.append("formal_authority_v6_interpreter_role_identity_split:" + str(role))
        ...
        if item.get("platform") == "POSIX_WSL":
            if wsl_orchestrator is not None and wsl_orchestrator != item.get("orchestrator_identity"):
                failures.append("formal_authority_v6_wsl_orchestrator_identity_split")
    ...
    if python3 and python314 and python3.get("resolved_target") != python314.get("resolved_target"):
        failures.append("formal_authority_v6_python_entry_target_split")
...
def _validate_production_runtime_dependencies(
    ...
    try:
        ...
        return [
            "formal_authority_v6_production_dependency_config_invalid:"
            + observation_id
        ]
    ...
    if not isinstance(value, list):
        return [
            "formal_authority_v6_production_dependency_collection_invalid:"
            + observation_id
        ]
    dependency_keys = {
    ...
    for entry in value:
        if not isinstance(entry, dict) or set(entry) != dependency_keys:
            failures.append(
                "formal_authority_v6_production_dependency_schema_invalid:"
                + observation_id
            )
            continue
        ...
        if not isinstance(relative, str) or not _safe_relative(relative):
            failures.append(
                "formal_authority_v6_production_dependency_path_invalid:"
                + observation_id
            )
            continue
    ...
    for relative in expected_paths:
        ...
        if source is None:
            failures.append(
                "formal_authority_v6_production_dependency_source_role_missing:"
                + observation_id + ":" + relative
            )
            continue
        expected_identity = {
        ...
        try:
            ...
            failures.append(
                "formal_authority_v6_production_dependency_live_recompute_failed:"
                + observation_id + ":" + relative
            )
            continue
        ...
        for phase in ("before", "after"):
            ...
            failures.extend(_identity_failures(
                identity,
                "formal_authority_v6_production_dependency_{}:{}:{}".format(
        ...
        if not _same(entry.get("signature_before"), entry.get("signature_after")):
            failures.append(
                "formal_authority_v6_production_dependency_runtime_drift:"
                + observation_id + ":" + relative
            )
...
def _validate_production_observations(
    ...
    if not isinstance(value, list):
        return ["formal_authority_v6_production_observations_invalid"]
    ...
    for item in value:
        if not isinstance(item, dict) or set(item) != keys:
            failures.append("formal_authority_v6_production_observation_schema_invalid")
            continue
        observation_id = item.get("observation_id")
        if not isinstance(observation_id, str) or not observation_id:
            failures.append(
                "formal_authority_v6_production_observation_id_invalid"
            )
            continue
        if observation_id in by_id:
            failures.append("formal_authority_v6_production_observation_duplicate")
            continue
        by_id[observation_id] = item
        definition = expected.get(observation_id)
        if definition is None:
            failures.append("formal_authority_v6_production_observation_unknown")
            continue
        ...
        for key, wanted in (
            ...
            if not _same(item.get(key), wanted):
                failures.append("formal_authority_v6_production_observation_mismatch:{}:{}".format(observation_id, key))
        for key in ("argv_sha256", "environment_sha256", "payload_sha256"):
            if not _valid_sha256(item.get(key)):
                failures.append("formal_authority_v6_production_observation_hash_invalid:{}:{}".format(observation_id, key))
        for key, material in (
            ("argv_sha256", item.get("argv")),
            ("environment_sha256", item.get("environment")),
            ...
            if _valid_sha256(item.get(key)) and item[key] != _canonical_sha256(material):
                failures.append("formal_authority_v6_production_observation_hash_mismatch:{}:{}".format(observation_id, key))
        ...
        if definition["execution_attempted"]:
            ...
            for payload_failure in production_payload_contract_failures(
                    definition, payload):
                failures.append(
                    "formal_authority_v6_production_payload_"
                    + payload_failure + ":"
                    + str(observation_id)
                )
            failures.extend(_interpreter_identity_failures(
                item.get("interpreter_identity"), "system_python314_target",
                "formal_authority_v6_production_interpreter_identity:" + str(observation_id),
            ))
            failures.extend(_orchestrator_identity_failures(
                item.get("orchestrator_identity"),
                "formal_authority_v6_production_orchestrator_identity:" + str(observation_id),
            ))
            try:
                ...
                expected_argv = None
                failures.append(
                    "formal_authority_v6_production_argv_recompute_failed:"
                    + str(observation_id)
                )
            if expected_argv is None or not _same(item.get("argv"), expected_argv):
                failures.append(
            ...
            if not _same(item.get("environment"), dict(CHILD_ENVIRONMENT)):
                failures.append(
                    "formal_authority_v6_production_environment_mismatch:"
                    + str(observation_id)
                )
            if isinstance(physical_by_id, Mapping):
                ...
                if (
                    ...
                    failures.append(
                        "formal_authority_v6_production_interpreter_identity_split:"
                        + str(observation_id)
                    )
                if (
                    not wsl_identities
                    ...
                    failures.append(
                        "formal_authority_v6_production_orchestrator_identity_split:"
                        + str(observation_id)
                    )
        else:
            if item.get("interpreter_identity") is not None or item.get("orchestrator_identity") is not None:
                failures.append("formal_authority_v6_static_observation_execution_identity_forbidden:" + str(observation_id))
            if item.get("argv") != ["NOT_EXECUTED_SAFETY_BOUNDARY"]:
                failures.append(
                    "formal_authority_v6_static_observation_argv_invalid:"
                    + str(observation_id)
                )
            if item.get("environment") != {}:
                failures.append(
                    "formal_authority_v6_static_observation_environment_invalid:"
                    + str(observation_id)
                )
        ...
        if not _stream_identity_valid(item.get("stdout")) or not _stream_identity_valid(item.get("stderr")):
            failures.append("formal_authority_v6_production_observation_stream_invalid:" + str(observation_id))
        try:
            ...
            failures.append(
                "formal_authority_v6_production_stream_recompute_failed:"
                + str(observation_id)
            )
    ...
    if set(by_id) != set(expected):
        failures.append("formal_authority_v6_production_observation_set_invalid")
    if value != sorted(
        value,
        key=lambda item: (
        ...
        failures.append("formal_authority_v6_production_observation_order_invalid")
    return failures
...
def _validate_predecessor_payload(payload: Any, policy: AuthorityPolicy) -> List[str]:
    if not isinstance(payload, dict):
        return ["formal_authority_v6_predecessor_payload_invalid"]
    ...
    for key in ("authority_id", "index_instance_id", "generation_id", "current_evidence_id"):
        if payload.get(key) != expected[key]:
            failures.append("formal_authority_v6_predecessor_semantic_mismatch:" + key)
    currents = [
    ...
    if len(currents) != 1:
        failures.append("formal_authority_v6_predecessor_current_count_invalid")
    elif currents[0].get("evidence_id") != expected["current_evidence_id"]:
        failures.append("formal_authority_v6_predecessor_current_mismatch")
    ...
    if payload.get("accepted_by_formal_field_evidence_consumer") is not False or payload.get("authorizes_field_delivery") is not False:
        failures.append("formal_authority_v6_predecessor_field_promotion_invalid")
    return failures
...
def _validate_source_roles(
    ...
    if not isinstance(value, list):
        return ["formal_authority_v6_source_roles_invalid"], {}
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict) or set(item) != {"role", "root_role", "path", "size_bytes", "sha256"}:
            failures.append("formal_authority_v6_source_role_invalid:" + str(index))
            continue
        if (
            ...
            failures.append(
                "formal_authority_v6_source_role_identity_type_invalid:"
                + str(index)
            )
            continue
        key = (item.get("root_role"), item.get("path"))
        if key in by_key:
            failures.append("formal_authority_v6_source_role_duplicate:" + repr(key))
            continue
        by_key[key] = item
        if expected.get(key) != item.get("role"):
            failures.append("formal_authority_v6_source_role_name_mismatch:" + repr(key))
        identity = {name: item.get(name) for name in ("root_role", "path", "size_bytes", "sha256")}
        failures.extend(_identity_failures(
            identity, "formal_authority_v6_source_role", with_root=True,
            allow_empty=(key in policy.allowed_empty_source_paths),
        ))
        try:
            actual = source_artifact_identity(workspace, key[0], key[1])
        except (OSError, UnicodeError, ValueError, TypeError):
            failures.append("formal_authority_v6_source_role_unreadable:" + repr(key))
            continue
        if actual != identity:
            failures.append("formal_authority_v6_source_role_identity_mismatch:" + repr(key))
        frozen = policy.frozen_source_identities.get(key)
        if frozen is not None and not _same(identity, frozen):
            failures.append("formal_authority_v6_frozen_source_identity_mismatch:" + repr(key))
    if set(by_key) != set(expected):
        failures.append("formal_authority_v6_source_role_set_invalid")
    if value != sorted(
        value,
        key=lambda item: (
        ...
        failures.append("formal_authority_v6_source_role_order_invalid")
    if any(
        str(item.get("path", "")).lower().startswith("evidence/")
        and "diagnostic" in str(item.get("path", "")).lower()
        for item in value if isinstance(item, dict)
    ):
        failures.append("formal_authority_v6_nonformal_diagnostic_source_forbidden")
    try:
        collect_host_perception_package_tree(workspace, policy)
    except (OSError, TypeError, UnicodeError, ValueError):
        failures.append("formal_authority_v6_host_package_tree_invalid")
...
def _validate_canonical(
    ...
    if not isinstance(payload, dict):
        return ["formal_authority_v6_canonical_schema_invalid"]
    ...
    failures: List[str] = []
    if not _same(payload, expected):
        failures.append("formal_authority_v6_canonical_mismatch")
    if not _valid_sha256(binding) or _canonical_sha256(without) != binding:
        failures.append("formal_authority_v6_canonical_binding_invalid")
    return failures
...
def _validate_report(
    ...
    if not isinstance(payload, dict):
        return ["formal_authority_v6_report_schema_invalid"]
    failures: List[str] = []
    expected_keys = {
    ...
    for key, expected in expected_scalars.items():
        if not _same(payload.get(key), expected):
            failures.append("formal_authority_v6_report_mismatch:" + key)
    ...
    if not isinstance(matrix, dict) or set(matrix) != matrix_keys:
        failures.append("formal_authority_v6_report_test_matrix_invalid")
    else:
        expected_logical = expected_logical_suite_records(workspace, source_roles, policy)
        if not _same(matrix.get("logical_suite_records"), expected_logical):
            failures.append("formal_authority_v6_logical_suite_records_mismatch")
        physical_failures, by_id = _validate_physical_records(
        ...
        if not _same(matrix.get("platform_composites"), expected_composites) or not all(item["validated_pass"] for item in expected_composites):
            failures.append("formal_authority_v6_platform_composite_invalid")
        ...
        for key, expected in expected_counts.items():
            if not _same(matrix.get(key), expected):
                failures.append("formal_authority_v6_report_test_count_mismatch:" + key)
    ...
    if not _valid_sha256(claimed) or _canonical_sha256(without) != claimed:
        failures.append("formal_authority_v6_report_binding_invalid")
    return failures
...
def validate_formal_admission_evidence_authority_v6(
    ...
    if failures:
        return result
    if not isinstance(payload, dict):
        failures.append("formal_authority_v6_payload_invalid")
        return result
    expected_keys = {
    ...
    if set(payload) != expected_keys:
        failures.append("formal_authority_v6_top_level_keys_invalid")
    expected_scalars = {
    ...
    for key, expected in expected_scalars.items():
        if not _same(payload.get(key), expected):
            failures.append("formal_authority_v6_top_level_mismatch:" + key)
    ...
    if payload.get("source_role_count") != len(source_roles):
        failures.append("formal_authority_v6_source_role_count_invalid")
    if payload.get("source_role_set_sha256") != _source_role_set_sha256(source_roles):
        failures.append("formal_authority_v6_source_role_sha256_invalid")

    entries = payload.get("entries")
    by_id: Dict[str, Mapping[str, Any]] = {}
    currents: List[Mapping[str, Any]] = []
    if not isinstance(entries, list) or len(entries) != 2:
        failures.append("formal_authority_v6_entry_count_invalid")
        entries = entries if isinstance(entries, list) else []
    for item in entries:
        if not isinstance(item, dict) or not isinstance(item.get("evidence_id"), str):
            failures.append("formal_authority_v6_entry_invalid")
            continue
        evidence_id = item["evidence_id"]
        if evidence_id in by_id:
            failures.append("formal_authority_v6_duplicate_evidence_id")
    ...
    if set(by_id) != {predecessor_id, CURRENT_EVIDENCE_ID}:
        failures.append("formal_authority_v6_entry_set_invalid")
    if len(currents) != 1:
        failures.append("formal_authority_v6_current_count_invalid")
    elif currents[0].get("evidence_id") != CURRENT_EVIDENCE_ID:
        failures.append("formal_authority_v6_current_marker_invalid")
    ...
    if old != expected_entries[0]:
        failures.append("formal_authority_v6_predecessor_entry_mismatch")
    ...
    if not isinstance(children, list) or len(children) != 1 or not isinstance(children[0], dict):
        ...
        if expected_child is None or child != expected_child:
            failures.append("formal_authority_v6_canonical_child_mismatch")
    ...
    for artifact_id, expected_identity, role in artifacts:
        try:
            ...
            failures.append("formal_authority_v6_artifact_unreadable:" + artifact_id)
            continue
        expected_file = {key: expected_identity[key] for key in ("root_role", "path", "size_bytes", "sha256")}
        if not _same(identity, expected_file):
            failures.append("formal_authority_v6_artifact_identity_mismatch:" + artifact_id)
        try:
            artifact_payload = _strict_json_bytes(raw)
        except (UnicodeError, ValueError, json.JSONDecodeError):
            failures.append("formal_authority_v6_artifact_strict_json_invalid:" + artifact_id)
            continue
        try:
            if role == "predecessor":
                ...
                for key, expected in (
                    ...
                    if not isinstance(artifact_payload, dict) or artifact_payload.get(key) != expected:
                        failures.append("formal_authority_v6_predecessor_report_mismatch:" + key)
                ...
                for key, expected in (
                    ...
                    if not isinstance(artifact_payload, dict) or artifact_payload.get(key) != expected:
                        failures.append("formal_authority_v6_predecessor_canonical_mismatch:" + key)
            elif role == "report":
            ...
            failures.append("formal_authority_v6_artifact_semantic_recompute_failed:" + artifact_id)
...
def load_and_resolve_formal_admission_evidence_authority_v6(
    ...
    failures: List[str] = []
    failures.extend(_policy_failures(policy))
    failures.extend(_identity_failures(index_trust_anchor, "formal_authority_v6_index_anchor"))
    if isinstance(index_trust_anchor, dict) and index_trust_anchor.get("path") != policy.index_relative_path:
        failures.append("formal_authority_v6_index_anchor_path_mismatch")
    ...
    if not failures:
        try:
            ...
            failures.append("formal_authority_v6_index_unreadable")
    if identity and identity != dict(index_trust_anchor):
        for key in ("path", "size_bytes", "sha256"):
            if identity.get(key) != index_trust_anchor.get(key):
                failures.append("formal_authority_v6_index_" + key + "_mismatch")
    payload = None
    if raw:
        try:
            payload = _strict_json_bytes(raw)
        except (UnicodeError, ValueError, json.JSONDecodeError):
            failures.append("formal_authority_v6_index_strict_json_invalid")
    validation = validate_formal_admission_evidence_authority_v6(workspace, payload, policy)
...
__all__ = [
    ...
    "expected_logical_suite_records", "load_and_resolve_formal_admission_evidence_authority_v6",
    "source_artifact_identity", "static_test_ids", "suite_inventory",
    "_expected_production_cli_argv",
    "validate_formal_admission_evidence_authority_v6", "write_json_exclusive",
]
